...
export class UsersCreaeditaComponent {
  ...
  tarjetaFormControl = new FormControl('', [
    Validators.pattern('[0-9]*'),
  ...
  ngOnInit(): void {
    ...
    this.form = this.formBuilder.group({
      ...
      adressUser: ['', Validators.required],
      cardUser: [''],
  ...
  aceptar() {
    if (this.form.valid) {
      ...
      this.users.adressUser = this.form.value.adressUser;
      if (this.edicion) {
    ...
    if (this.edicion) {
      this.cS.listId(this.id).subscribe((data) => {
        this.form = new FormGroup({
          ...
          adressUser: new FormControl(data.adressUser, Validators.required),
        });
      });
    }
  }
